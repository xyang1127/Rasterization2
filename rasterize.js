    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                imgPath = "https://ncsucgclass.github.io/prog4/" + inputTriangles[whichSet].material.texture;
                inputTriangles[whichSet].glTexture = loadImg(imgPath);

                imgPath = "https://ncsucgclass.github.io/prog4/lightmap.gif";
                inputTriangles[whichSet].glLight = loadImg(imgPath);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            // ----------------------
            vec4 tColor = texture2D(sampler, vTexturePosition);
            vec4 lightColor = texture2D(lightSampler, vTexturePosition);

            vec4 color = vec4(0,0,0,1);
            if(unlit)
                color = tColor;
            else if(lightmapped)
                color = tColor * lightColor;
            else if(blinn)
                color = tColor * vec4(colorOut, 1.0);

            if(transparency)
                color = vec4(color.rgb, color.a * uAlpha);
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                // texture -----------------------
                texAttribLoc = gl.getAttribLocation(shaderProgram, "aTexturePosition");
                gl.enableVertexAttribArray(texAttribLoc);
                
                samplerLoc = gl.getUniformLocation(shaderProgram,"sampler");
                gl.uniform1i(samplerLoc, 0);  // texture unit 0

                lightMapLoc = gl.getUniformLocation(shaderProgram, "lightSampler");
                gl.uniform1i(lightMapLoc, 1);  // texture unit 1

                alphaLoc = gl.getUniformLocation(shaderProgram, "uAlpha");

                unlitLoc = gl.getUniformLocation(shaderProgram, "unlit");
                lightmappedLoc = gl.getUniformLocation(shaderProgram, "lightmapped");
                blinnLoc = gl.getUniformLocation(shaderProgram, "blinn");
                transparencyLoc = gl.getUniformLocation(shaderProgram, "transparency");
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        gl.bindTexture(gl.TEXTURE_2D, inputTriangles[whichTriSet].glLight);

        gl.uniform1f(alphaLoc, inputTriangles[whichTriSet].material.alpha);

        gl.uniform1i(unlitLoc, unlit);
        gl.uniform1i(lightmappedLoc, lightmapped);
        gl.uniform1i(blinnLoc, blinn);
